/** 判断 SSE 解析后的 payload 是否为知识库 meta（勿写入对话正文）。 */
export function isKnowledgeMetaPayload(chunk) {
...
export function parseKnowledgeMetaPayload(chunk) {
  if (!chunk) {
    return null
  }
  let trimmed = chunk.trim()
  ...
  try {
    ...
    if (parsed?.citations?.length) {
      parsed.citations = parsed.citations.map((item) => ({
        ...
        vectorScore: item.vectorScore != null ? Number(item.vectorScore) : null,
      }))
    }
    return parsed
  } catch {
...
export function stripKnowledgeMetaFromText(fullText) {
...
/** 0–100 展示；小于 1% 时保留一位小数，避免 0.4% 被四舍五入成 0%。 */
...
export function normalizeCitation(citation) {
  ...
  return {
    ...
    documentName: citation.documentName || citation.fileName || '',
